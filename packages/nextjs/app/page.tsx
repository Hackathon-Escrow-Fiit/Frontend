...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    ...
    router.push(currentName ? "/messages" : "/setup");
  }, [wantsToEnter, isConnected, address, currentName, isLoadingName, router]);

  const handleEnter = (openConnectModal: () => void, connected: boolean) => {
          ...
          <div className="flex gap-3">
            <ConnectButton.Custom>
              {({ openConnectModal, mounted, account, chain }) => {
                ...
                return (
                  <button onClick={() => handleEnter(openConnectModal, connected)} className="btn btn-primary gap-2">
            ...
            <ConnectButton.Custom>
              {({ openConnectModal, mounted, account, chain }) => {
                const connected = !!(mounted && account && chain);
                return (
                  <button onClick={() => handleEnter(openConnectModal, connected)} className="btn btn-outline">
                    Find Work
                  </button>
                );
              }}
