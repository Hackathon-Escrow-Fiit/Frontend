...
export default function TaskViewPage() {
  ...
  useEffect(() => {
    fetch(`${backendUrl}/rate-task/${id}`)
      ...
      .then(data => { if (data) setTaskRating(data); })
      .catch(() => {});
  ...
  const hasCounterparty =
    !!counterpartyAddr && counterpartyAddr !== "0x0000000000000000000000000000000000000000";
                ...
                <>
                  ...
                  <div className="bg-base-100 rounded-2xl border border-base-200 overflow-hidden">
                    ...
                    >
                      ...
                      <span className="text-base-content/40 text-sm">{reportExpanded ? "â–² collapse" : "â–¼ expand"}</span>
                    </button>
                    {reportExpanded && (
        ...
        {view === "review" && (() => {
          ...
          return (
            <div className="flex gap-5 items-start">
              <div className="flex-1 min-w-0 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="ml-auto">
                    {noSubmission && <span className="bg-base-200 text-base-content/50 text-xs font-semibold px-3 py-1.5 rounded-full">No submission yet</span>}
                    {isEvaluating && <span className="flex items-center gap-2 bg-warning/10 text-warning text-xs font-semibold px-3 py-1.5 rounded-full"><span className="loading loading-spinner loading-xs" /> AI Evaluating…</span>}
                    {isError && <span className="badge badge-error badge-lg">Evaluation Error</span>}
                  </div>
                </div>
                ...
                {noSubmission && (
                  <div className="bg-base-100 rounded-2xl border border-base-200 p-12 flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-base-content/50 max-w-sm">The freelancer has not submitted their work. Review actions will unlock once a submission is received.</p>
                  </div>
                )}
                ...
                {isEvaluating && (
                  <div className="bg-base-100 rounded-2xl border border-base-200 p-12 flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-base-content/50 max-w-sm">Analyzing code quality, security, and task completion. Updates automatically.</p>
                  </div>
                )}
                ...
                {isReady && report && (
                  <>
                    {recStyle && (
                      <div className={`rounded-2xl border ${recStyle.bg} ${recStyle.border} p-5 flex items-center gap-4`}>
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold shrink-0 ${recStyle.bg} border ${recStyle.border} ${recStyle.text}`}>{recStyle.icon}</div>
                        <div className="flex-1 min-w-0">
                          <p className={`font-bold text-lg ${recStyle.text}`}>{recStyle.label}</p>
                          <p className="text-xs text-base-content/50 mt-0.5">
                            Confidence: <span className={`font-semibold ${confColor}`}>{report.confidence_score}%</span>
                              ...
                              <div className="space-y-1">
                                {report.files_submitted.map(f => (
                                  <div key={f} className="flex items-center gap-2 bg-success/5 border border-success/20 rounded-lg px-3 py-1.5">
                                    ...
                                    <span className="font-mono text-xs text-base-content/80 truncate flex-1">{f}</span>
                                    <a href={`${backendUrl}/files/${id}/${encodeURIComponent(f)}`} download={f} className="btn btn-ghost btn-xs btn-square shrink-0"><ArrowDownTrayIcon className="w-3.5 h-3.5 text-base-content/50" /></a>
                                  </div>
                                ))}
                              </div>
                            </div>
                    ...
                    {(report.requirements_check ?? []).length > 0 && (
                      <div className="bg-base-100 rounded-2xl border border-base-200 p-6">
                        <div className="flex items-center justify-between mb-4">
                          <h2 className="font-bold text-base-content">Task Requirements</h2>
                          <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${metCount === totalCount ? "bg-success/15 text-success" : metCount === 0 ? "bg-error/15 text-error" : "bg-warning/15 text-warning"}`}>{metCount}/{totalCount} met</span>
                        </div>
                        <div className="space-y-2">
                          {report.requirements_check.map((req, i) => (
                            <div key={i} className={`rounded-xl p-4 border ${req.met ? "bg-success/5 border-success/20" : "bg-error/5 border-error/20"}`}>
                              <div className="flex items-start gap-3">
                                <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold ${req.met ? "bg-success text-success-content" : "bg-error text-error-content"}`}>{req.met ? "✓" : "✗"}</div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-sm font-semibold text-base-content">{req.requirement}</p>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="flex gap-1.5">
                            {["critical","high","medium","low"].map(sev => {
                              ...
                              if (!count) return null;
                              const s = sevStyle(sev);
                              return <span key={sev} className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${s.badge}`}>{count} {sev}</span>;
                            })}
                          </div>
                        </div>
                        <div className="space-y-3">
                            ...
                            return (
                              <div key={i} className="rounded-xl border border-base-200 overflow-hidden">
                                <div className="flex items-center gap-0 bg-base-200">
                                  <div className={`w-1 self-stretch ${s.bar}`} />
                                  <div className="flex items-center gap-3 px-4 py-3 flex-1 min-w-0">
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase shrink-0 ${s.badge}`}>{issue.severity}</span>
                                    <p className="text-sm font-semibold text-base-content truncate">{issue.issue}</p>
                                    <div className="ml-auto flex items-center gap-1.5 shrink-0">
                                      <span className="text-[10px] font-mono bg-base-300 text-base-content/70 px-2 py-0.5 rounded">{issue.file}</span>
                                      <span className="text-[10px] font-mono bg-base-300 text-base-content/50 px-1.5 py-0.5 rounded">:{issue.line}</span>
                                    </div>
                                  </div>
                                </div>
                                <div className="px-5 py-3 bg-base-100">
                                  ...
                                  {issue.snippet && (
                                    <div className="mt-2 bg-base-content/5 border border-base-200 rounded-lg px-3 py-2 font-mono text-xs text-base-content/80 overflow-x-auto">
                                      <span className="text-base-content/30 select-none mr-2">{issue.line}</span>{issue.snippet}
                                    </div>
                                  )}
                                </div>
                              </div>
                    ...
                    {submittedFiles.length > 0 && (
                      <div className="bg-base-100 rounded-2xl border border-base-200 p-5">
                        <p className="text-[10px] font-bold tracking-widest text-base-content/40 uppercase mb-3">Submission Files</p>
                        <div className="space-y-2">
                          {submittedFiles.map(f => (
                            <div key={f.filename} className="flex items-center gap-3 border border-base-200 rounded-xl px-4 py-3">
                              ...
                              <a href={`${backendUrl}/files/${id}/${encodeURIComponent(f.filename)}`} download={f.filename} className="btn btn-ghost btn-xs btn-square shrink-0">
                                <ArrowDownTrayIcon className="w-4 h-4 text-base-content/50" />
                              </a>
                            </div>
                          ))}
                    ...
                    <div className="bg-base-100 rounded-2xl border border-base-200 overflow-hidden">
                      <button onClick={() => setReportExpanded(p => !p)} className="w-full flex items-center justify-between px-6 py-4 hover:bg-base-200/50 transition-colors">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center shrink-0"><span className="text-[10px] font-bold text-primary-content">AI</span></div>
                          <span className="font-bold text-base-content">Full AI Evaluation Report</span>
                        </div>
                      ...
                      {reportExpanded && (
                        <div className="px-6 pb-6">
                          <div className="bg-base-200 rounded-xl p-4 max-h-[500px] overflow-y-auto">
                            <pre className="text-sm text-base-content/80 leading-relaxed whitespace-pre-wrap font-sans">{report.detailed_report}</pre>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className="w-60 shrink-0 space-y-3">
                <div className="bg-base-100 rounded-2xl border border-base-200 p-5">
                  <h2 className="text-xs font-bold tracking-widest text-base-content/60 uppercase mb-4">Review Actions</h2>
                  {(noSubmission || isEvaluating) ? (
                    <div className="space-y-2">
                      <div className="rounded-xl border-2 border-dashed border-base-300 p-4 text-center mb-1">
                        <p className="text-xs text-base-content/40 leading-relaxed">
                          {noSubmission ? "Buttons unlock once the freelancer submits their solution." : "Waiting for AI evaluation to complete."}
                        </p>
                      </div>
                      <button disabled className="btn btn-primary w-full gap-2 h-14 text-sm opacity-40 cursor-not-allowed"><CurrencyDollarIcon className="w-5 h-5" /> Approve &amp; Release Payment</button>
                      <button disabled className="btn btn-outline w-full gap-2 opacity-40 cursor-not-allowed"><ArrowPathIcon className="w-4 h-4" /> Request Changes</button>
                      <button disabled className="btn w-full gap-2 opacity-40 cursor-not-allowed bg-orange-50 border-orange-200 text-orange-600"><ExclamationTriangleIcon className="w-4 h-4" /> Open Dispute</button>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <button className="btn btn-primary w-full gap-2 h-14 text-sm" onClick={handleApprove} disabled={approveLoading || approved}>
                        {approveLoading ? <><span className="loading loading-spinner loading-xs" /> Processing…</> : approved ? <><CheckCircleIcon className="w-5 h-5" /> Payment Released</> : <><CurrencyDollarIcon className="w-5 h-5" /> Approve &amp; Release Payment</>}
                      </button>
                      <button className="btn btn-outline w-full gap-2"><ArrowPathIcon className="w-4 h-4" /> Request Changes</button>
                      <Link href={`/disputes/${id}/defense`} className="btn w-full gap-2 bg-orange-50 border-orange-200 text-orange-600 hover:bg-orange-100 hover:border-orange-300"><ExclamationTriangleIcon className="w-4 h-4" /> Open Dispute</Link>
                    </div>
                  )}
                  <p className="text-[10px] text-base-content/40 leading-relaxed mt-3 text-center">Only use Dispute if direct communication fails.</p>
                </div>
                  ...
                  <div className="bg-base-100 rounded-2xl border border-base-200 p-5">
                    ...
                    <div className="space-y-2">
                      {["critical","high","medium","low"].map(sev => {
                        const count = (report?.code_issues ?? []).filter(i => i.severity === sev).length;
                        if (!count) return null;
                        const sevColors: Record<string,string> = { critical:"text-error", high:"text-warning", medium:"text-info", low:"text-base-content/60" };
                        const sevBadges: Record<string,string> = { critical:"bg-error text-error-content", high:"bg-warning text-warning-content", medium:"bg-info text-info-content", low:"bg-base-300 text-base-content" };
                        return <div key={sev} className="flex items-center justify-between"><span className={`text-xs font-semibold capitalize ${sevColors[sev]}`}>{sev}</span><span className={`text-xs font-bold px-2 py-0.5 rounded-full ${sevBadges[sev]}`}>{count}</span></div>;
                      })}
                    </div>
                  </div>
                )}
                ...
                <div className="bg-base-200/50 rounded-2xl border border-base-200 p-5">
                  ...
                  <p className="text-xs text-base-content/55 leading-relaxed">If the AI result seems wrong, escalate to DAO for a human review.</p>
                </div>
              </div>
            </div>
          );
